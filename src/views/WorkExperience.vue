	...
	<section class="mb-8">
		...
		<div class="md:grid md:grid-cols-2 gap-4 space-y-4 md:space-y-0">
			<div v-for="item in data?.[$i18n.locale]" :key="item.id">
				<div
					class="relative bg-primary-600 px-4 pt-8 pb-12 rounded-t-lg text-center"
				>
					...
					<div
						class="company-logo rounded-full h-12 w-12 bg-white flex items-center justify-center"
					>
						<img
					...
					<div>
						<div class="text-center">
							<button
								...
								:class="{
									'arrow-wrapper--rotate-180': showCompanies.includes(item.id),
								}"
							...
							>
								...
								<div class="flex flex-wrap">
									...
									>
										<span>{{ skill }}</span>
										<span
											v-if="skillIndex < project.skills.length - 1"
											class="mx-1"
											>•</span
										>
									</div>
...
export default defineComponent({
	...
	setup: () => {
		...
		function toggleShowCompany(idToRemove: string) {
			const currentIndex = showCompanies.value.findIndex(
